...
_prepare_fonts() {
	...
	TEMP_FONTS_ARCHIVE=$(_mktemp_file)
	...
	BROWSER_URL=$(_get_item "browser_download_url" "${TEMP_LATEST_INFO}")
	say -n -cyan "Downloading: "
	say -normal "${BROWSER_URL}"
	_download "${BROWSER_URL}" "${TEMP_FONTS_ARCHIVE}"
...
_prepare() {
	...
	_extract "${TEMP_FONTS_ARCHIVE}" "${TEMP_RELEASE_DIR}"

	if [ -d "${TEMP_RELEASE_DIR}/fonts" ]; then
		if [ -d "${TEMP_RELEASE_DIR}/fonts/webfonts" ]; then
			say -green "Updating webfonts..."
			mv -un "${TEMP_RELEASE_DIR}/fonts/webfonts" "${LATEST_RELEASE_DIR}"
		fi
		if [ -d "${TEMP_RELEASE_DIR}/fonts/variable" ]; then
			say -green "Updating variable..."
			mv -un "${TEMP_RELEASE_DIR}/fonts/variable" "${LATEST_RELEASE_DIR}"
		fi
		if [ -d "${TEMP_RELEASE_DIR}/fonts/ttf" ]; then
			say -green "Updating ttf..."
			mv -un "${TEMP_RELEASE_DIR}/fonts/ttf" "${LATEST_RELEASE_DIR}"
		fi
